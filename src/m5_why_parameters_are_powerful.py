...
def main():
    ...
    #even_better_draw_circles(rg.Point(100, 50), 7, 10, 'red', 1)
    #even_better_draw_circles(rg.Point(-200, 0), 15, 19, 'green', 10)
    my_circles(rg.Point(0, 0), 5, 60, 10, 50)
    window.update()
...
def even_better_draw_circles(point, circles, radius, color, thickness):
    turtle = rg.SimpleTurtle()
...
###############################################################################
def my_circles(point, radius, turn, thickness, circles ):
    turtle = rg.SimpleTurtle()
    ...
    for k in range(1, circles):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.right(turn)
        turtle.forward(radius)
        turtle.pen = rg.Pen(seq[k % 6], thickness)
        turtle.pen_down()
        turtle.draw_circle(radius * k)
# -----------------------------------------------------------------------------
